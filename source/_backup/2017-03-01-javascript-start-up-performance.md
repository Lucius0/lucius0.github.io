---
layout: post
title:  "JavaScript 启动性能瓶颈分析与解决方案"
date:   2017-03-01
categories: front-end
permalink: /archivers/javascript-start-up-performance
tags: javascript
---
...
![](/images/javascript/js-08.png)

**大部分应用在桌面浏览器中会耗费约 4 秒的时间进行 JavaScript 启动阶段（语法解析、编译、执行）：**

![](/images/javascript/js-09.jpg)
...
![](/images/javascript/js-11.jpg)

### Chrome Tracing

打开 about:tracing 页面，Chrome 提供的底层的追踪工具允许我们使用disabled-by-default-v8.runtime_stats来深度了解 V8 的时间消耗情况。V8 也提供了[详细的指南](https://docs.google.com/presentation/d/1Lq2DD28CGa7bxawVH_2OcmyiTiBn74dvC6vn2essroY/edit#slide=id.g1a504e63c9_2_84)来介绍如何使用这个功能。

![](/images/javascript/js-12.jpg)

### WebPageTest

![](/images/javascript/js-13.png)
...
![](/images/javascript/js-15.jpg)

### DeviceTiming

Etsy 的 [DeviceTiming](https://github.com/danielmendel/DeviceTiming) 工具能够模拟某些受限环境来评估页面的语法解析与执行时间。其将本地脚本包裹在了某个仪表工具代码内从而使我们的页面能够模拟从不同的设备中访问。可以阅读 [Daniel Espeset 的Benchmarking JS Parsing and Execution on Mobile Devices](http://talks.desp.in/unpacking-the-black-box) 一文来了解更详细的使用方式。

![](/images/javascript/js-16.jpg)
...
![](/images/javascript/js-18.jpg)

本部分我们就会对过去几年中我们使用的提升语法解析与编译时间的技巧进行阐述。

### 代码缓存

![](/images/javascript/js-19.png)
...
![](/images/javascript/js-20.png)

这个特性倒是第一次提及，因此 V8 会允许所有的脚本，即使阻塞型的`<script src=''>`脚本也可以由后台线程进行解析。不过缺陷就是目前仅有一个 streaming 后台线程存在，
因此我们建议首先解析大的、关键性的脚本。在实践中，我们建议将`<script defer>`添加到`<head>`块内，这样浏览器引擎就能够尽早地发现需要解析的脚本，然后将其分配给后台线程进行处理。我们也可以查看 DevTools Timeline 来确定脚本是否被后台解析，特别是当你存在某个关键性脚本需要解析的时候，更需要确定该脚本是由 streaming 线程解析的。

![](/images/javascript/js-21.png)
...
转自：[https://segmentfault.com/a/1190000008340139#articleHeader10](https://segmentfault.com/a/1190000008340139#articleHeader10)